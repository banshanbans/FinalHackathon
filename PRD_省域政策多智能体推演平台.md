...
```text
政策输入 → 中央解读顺序确认 → 实验设计（0–3 事件）→ 基线顺序冻结
...
```text
政策输入 → 中央解读顺序确认 → 实验设计 → 基线顺序冻结 → 推演运行 → 结果复盘
```
...
运行环境约束固定为：本地 Web/API 启动只允许 `fake` Mock Provider；公网演示默认使用 `cache` Provider。命中时回放已验证结构化输出；修改比例或其他语义输入导致 miss 时，服务端调用已批准 DeepSeek `live` Provider，经 Schema、主体身份和资源约束校验后原子回写 Luna 缓存。服务端运行模式是唯一真相，客户端不得在请求中切换 Provider。DeepSeek 失败时依产品契约显式 fallback，不将 Fake 输出写入 Luna 缓存。

M34 Luna 决策缓存必须使用带 `m34-decision-quality-v1` 标记的 v3 envelope。Cache 命中后仍须重新执行身份、消息授权、会话状态、资源额度和互动一致性校验；`initiate` 无消息、`respond` 无合法回应、被动决策无原因、多对手交易消息或未授权 session 均视为无效缓存。只有完整校验通过的 live 输出可回写；默认路径验收必须同时核对消息、会话、空发起和无理由被动决策，不能只统计调用与 fallback。
...
继续使用经过来源、31 省绑定和几何签名校验的离线矢量地图。所有全国地图必须完整展示港澳台上下文，并在地图画布左下角固定显示同一冻结自然资源部标准地图中的南海诸岛附图。附图固定于地图视口，不跟随俯视、侧视、缩放、平移或 A/B 视角变化，不得被右下角图例覆盖；它是地图内制图内容而不是说明卡。附图使用方角虚线框且没有卡片底色、阴影或外置标题牌；内部“南海诸岛”名称、岛礁与断续线只能裁切自冻结 GS(2016)1609 标准地图原件，不得手工重绘。中国地图本体还必须按获批参考图的空间比例显示从台湾、海南下方向南延伸的高对比南海断续线；该线从同一冻结原件机械分离并锚定到地图地理坐标，不是图例或屏幕固定叠层。主地图使用从官方断续符号机械求取的细长圆头中心线，不直接渲染粗重填充字形；其展示范围按台湾东侧、南海深处与海南西南侧的 U 形相对位置校准，不增加人工补点。拖动、缩放、俯视、侧视、分支或 A/B 切换时，主地图断续线必须与中国地图接受相同相机变换，并与台湾、海南保持不变的相对关系。南海诸岛只作版图展示，不进入 31 省计算、指标色阶、事件、Agent、关系线、比较或点击详情。电池节点和车企活动是数据覆盖层，不修改行政区边界。
...
正式模块为 `apps/presentation`，入口为 `/experiments/:id/present`。一张全屏画布通过顶部 HUD、右侧功能坞、浮动 Tab、模态窗、侧滑详情和底部时间轴完成实验配置、A/B 实验设计、运行、回放、结果对照与 Evidence 下钻；中央解读确认和基线冻结保留为顺序服务门禁，不再占用独立页面。
...
Presentation 主地图支持自动镜头、全国俯视与因果侧视。自动镜头在“行动、回应”节拍以倾斜镜头突出当前主体和关系路径，随后在结算恢复全国俯视。差值、季度结算和年度比较不得使用侧视，确保同域色阶和省域空间比较不被透视误导。该能力只属于展示层，不增加业务状态或模型调用。
