...
export default function ComparePage() {
  ...
  return <div className="compare-page page-stack">
    ...
    <section className="dual-map-grid">
      <article className="card comparison-map-card"><div className="card-heading"><div><span className="branch-label control">原始方案</span><h2>政策基准</h2></div><span>企业参与指数</span></div><Suspense fallback={<div className="map-loading"><span className="spinner" /></div>}><ProvinceMap compact metricLabel="企业参与指数" onSelect={select} profiles={flow.profiles} selectedCode={selectedCode} values={controlValues} /></Suspense></article>
      <article className="card comparison-map-card"><div className="card-heading"><div><span className="branch-label treatment">干预方案</span><h2>政策调整</h2></div><span>企业参与指数</span></div><Suspense fallback={<div className="map-loading"><span className="spinner" /></div>}><ProvinceMap compact metricLabel="企业参与指数" onSelect={select} profiles={flow.profiles} selectedCode={selectedCode} values={treatmentValues} /></Suspense></article>
    </section>
