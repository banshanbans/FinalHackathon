...
PolicyScope V2 是面向国务院层面政策统筹人员的“制造业设备更新政企互动 Agent 推演台”。它用于观察中央政策、地方工具与企业行动如何相互作用，并通过同源情景实验呈现政策收益、区域差异与财政代价。
...
- 全国页与 A/B 页共用本地 ECharts SVG 地图。资源源自自然资源部标准地图 GS(2016)1609，来源、校验和转换过程见 [地图说明](./apps/web/src/assets/maps/README.md)。公开发布仍须完成地图合规复核。
- 默认演示模式为 `cache`，测试模式为 `fake`，`live` 是可选增强且不构成交付依赖。
- V1 回滚点为 Git 提交 `6ea8e9d`。
...
- 地图 EPS 转 SVG 时保留边界几何，并以稳定省级代码标注 31 个可交互区域；公开使用前仍须核验标准地图使用方式、审图号标注和编辑后审核要求。
...
- 地图已完成来源、审图号、几何和 31 省技术校验，但公开发布前仍有人工合规复核门禁。
