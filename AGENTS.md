...
本文件适用于当前目录及全部子目录，用于约束参与 PolicyScope V3.1 的 Codex、自动化开发 Agent 和人工开发者。
...
- 默认新能源汽车场景必须有完整预生成缓存。
- 本地 Web/API 启动固定使用 `fake` Mock Provider，不得调用线上模型；线上部署固定使用 `live` Provider。
- `cache` 仅用于预计算、回归验证和可复现演示流程，不作为本地交互启动模式。
...
V3.1 M21–M28 已完成并冻结。下一步按 M29 采集和迁移省级真实事实层；不得用当前代理指数冒充真实事实，也不得回写 V3.0 冻结数据。
