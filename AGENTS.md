...
本文件适用于当前目录及其全部子目录，用于约束参与 PolicyScope V2 的 Codex、自动化开发 Agent 和人工开发者。

---

## 1. 当前阶段与工作门禁

当前仓库已完成 V2 原地迁移和产品 QA；V1 回滚基线保留在 Git 提交 `12456a3`。V2 的目标产品是：
...
用户输入中央目标
  ...
  → 用户审批
  → 31 个省级 Agent 选择地方工具
  → 每省 6 类企业群体 Agent 反馈行动
  → 确定性环境计算结果和机制贡献
  → 中央 Agent 提出干预建议
  → 用户批准、修改或拒绝
  → 同一 T3 Checkpoint 派生 Control/Treatment
  → T5 比较企业行为迁移、地区差异和政策代价
```
...
省级 Agent 生成地方政策工具组合、执行强度、配套资源、目标企业群体、合作或竞争策略、中央支持请求和结构化摘要。
...
- 企业行动、行为迁移、地区差异、财政压力和机制贡献。
- 四个核心路由、河南省企详情抽屉、方法与证据抽屉。
...
```text
T0 中央目标 → 中央结构化政策 → 用户审批
T1 31 个省级 Agent 生成地方工具组合
T2 31 次企业批量决策 → 186 个 Action → 环境计算
T3 31 个省级 Agent 生成地方反馈 → 汇总证据 → 冻结 Checkpoint → 中央建议 → 用户审批/修改/拒绝
T4 Control/Treatment 中省级和企业主体分别响应
T5 环境结算 → 企业行为迁移 → 全国 A/B → 中央复盘
```
...
### 8.4 `WorldState` 与 `ComparisonResult`
...
- ComparisonResult 必须包含企业动作迁移矩阵、重点企业群体变化、地区排行、企业层机制贡献和中央复盘。
...
- 现有 REST 资源路径尽量保持，DTO 通过版本升级迁移。
- 新增企业类型元数据接口，不允许前端硬编码语义规则。
...
- 企业事件至少包括批量决策开始、完成、fallback、企业聚合更新和地方反馈。
...
- `?province=41`：河南省企详情抽屉，显示六类企业动作、地方工具和机制贡献。
...
- 审批链、双地图、企业迁移和证据抽屉。
- 禁止文案扫描、单位扫描和无效链接扫描。
- E2E 主路径：创建政策 → 批准 → T1–T3 → 河南详情 → 审批干预 → 双分支 → A/B 与证据。
...
整个 V2 完成标准以 `DEVELOPMENT_PLAN.md` 的 Definition of Done 为准，不得自行降低。

---

## 18. 当前唯一下一步

保持比赛产品冻结，不扩展 P0 范围。地图可随比赛版直接上线；后续地图变更必须继续通过来源、省域绑定、几何签名和 31 省完整性校验。
